# Thesis_code_Dimitar
# Privacy-Utility Trade-off in B2B Telecom Churn Prediction

This repository contains the code for the master’s thesis studying how privacy-preserving methods affect churn prediction and retention targeting.

The project compares logistic regression, gradient boosting, and DP-SGD under multiple privacy levels, and evaluates both model performance and business value using e-Profit.

## Dataset
The dataset used in this project is uploaded in the repository / stored separately and is based on a real-world B2B telecom churn dataset.

## How to run
1. Install the required packages.
2. Place the dataset in the correct folder(the same folder where the notebook is located).
3. Open the notebook and run the cells in order.
